...
function processImageToGioye(imageData, bitDepth, width, height) {
    ...
    if (bitDepth === 16) {
        ...
        for (let i = 0; i < imageData.length; i += 4) {
            const val = Math.round((imageData[i] + imageData[i + 1] + imageData[i + 2]) / 3);
            if (val === prev) {
                count++;
            } else {
                if (prev !== null) pixelData.push(`${count} ${prev}`);
                prev = val;
                count = 1;
            }
        }
        ...
        for (let i = 0; i < imageData.length; i += 4) {
            ...
            if (JSON.stringify(pixel) === JSON.stringify(prev)) {
                count++;
            } else {
                if (prev !== null) pixelData.push(`${prev.join(' ')} ${count}`);
                prev = pixel;
                count = 1;
            }
        }
...
function decodeImage(file, format, outputFile) {
    ...
    if (isBinary) {
        ...
        const bytesPerPixel = { 1: 1, 8: 1, 16: 2, 24: 3, 32: 4, 40: 5 }[bitDepth];
    ...
    for (const pixel of pixels) {
        let r = 0, g = 0, b = 0, a = 255, repeat = 1;
...
program
    ...
    .version('2.1.0'); // Updated version to 2.1.0

program
    .command('encodecsv')
    .description('Encode PNG to Gioyeimg CSV format')
    .argument('<file>')
    .option('-e, --bitdepth <number>', 'Bit depth of the output (1, 8, 16, 24, 32, 40)', parseInt, 32)
    .option('-o, --output <file>', 'Output file name')
    .action((file, options) => {
        if (![1, 8, 16, 24, 32, 40].includes(options.bitdepth)) {
            console.error('Error: Invalid bit depth. Supported bit depths are 1, 8, 16, 24, 32, 40.');
            return;
        }
        encodeCSV(file, options.bitdepth, options.output);
    });
...
program
    ...
    .action((file, options) => {
        ...
        encodeBinary(file, options.bitdepth, options.output);
    });
...
program
    ...
    .argument('<file>')
    .option('-d, --format <format>', 'Output image format (png or jpeg)', 'png')
    .option('-o, --output <file>', 'Output file name')
    .action((file, options) => {
